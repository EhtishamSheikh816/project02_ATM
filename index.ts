...
if (pinCode.codes === code) {
  ...
  if (atm.action === "Deposit money") {
    ...
    if (deposits.money <= 50000) {
      ...
      console.log(
        chalk.blueBright(
          `You have successfully deposited $${deposits.money}.Your current balance is $${cash}`
        )
      );
    } else {
      console.log(
        chalk.red(
          "Sorry, the maximum amount that can be deposited in one time is $50,000."
        )
      );
    }
    ...
    if (withdrawal.amount <= cash) {
      ...
      console.log(
        chalk.blueBright(
          `Your Withdrawal of $${withdrawal.amount} was successfull.`
        )
      );
    } else {
